  ...
  return (
    <div>
      ...
      >
        ...
        />
        <div
          className="pointer-events-none absolute left-0 top-[15px] hidden h-px bg-[#1863dc] transition-[width] duration-500 sm:block"
        ...
        {steps.map((step, index) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  'relative z-10 mt-0.5 h-[11px] w-[11px] shrink-0 rounded-full border-2 bg-white transition-colors',
                  selected ? 'border-[#1863dc] bg-[#1863dc]' : 'border-[#d9d9dd]',
                )}
              />
              <span
                className={cn(
                  'text-base leading-snug transition-colors sm:mt-4 sm:text-sm md:text-base',
      ...
      <div
        ...
        className="mt-10 max-w-2xl sm:mx-auto sm:text-center"
      >
        <p className="text-xl leading-relaxed text-neutral-600 md:text-2xl">{item.copy}</p>
      </div>
    </div>
  );
}
...
function ModelExplainer() {
  ...
  return (
    <>
      <Accordion type="single" defaultValue="0" className="lg:hidden">
        {model.map((entry, index) => (
          <AccordionItem key={entry.title} value={String(index)} className="border-[#d9d9dd]">
            <AccordionTrigger className="items-start py-7 text-left hover:no-underline [&>svg]:mt-2 [&>svg]:text-[#1863dc]">
      ...
      <div
        className="hidden h-[32rem] gap-3 lg:grid"
        style={{
          gridTemplateColumns: model.map((_, index) => (index === active ? '2.4fr' : '0.85fr')).join(' '),
        }}
      >
        {model.map((entry, index) => {
          const selected = index === active;
          return (
            <button
              ...
              onClick={() => setActive(index)}
              className={cn(
                'flex h-full flex-col overflow-hidden rounded-[22px] p-8 text-left transition-[background-color,color] duration-300',
                selected
                  ? 'bg-[#1863dc] text-white'
                  : 'bg-[#f1f5ff] text-[#17171c] hover:bg-[#e4ecfb]',
              )}
            >
              <span className="display text-2xl leading-[1.1] xl:text-3xl">{entry.title}</span>
              <span
                className={cn(
                  'mt-8 block max-w-lg text-2xl leading-[1.2] tracking-[-0.03em] xl:text-3xl',
                  selected ? 'text-white' : 'sr-only',
                )}
              >
                {entry.why}
              </span>
              <span
                className={cn(
                  'mt-6 block max-w-lg text-base leading-relaxed xl:text-lg',
                  selected ? 'text-white/75' : 'sr-only',
                )}
              >
                {entry.what}
              </span>
