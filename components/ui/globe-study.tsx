...
(function(){
  ...
  Surface.prototype.resize = function(){
    var cssW = this.el.clientWidth || 0, cssH = this.el.clientHeight || 0;
    if (!cssW || !cssH) return false;
    var dpr = Math.min(1.25, window.devicePixelRatio || 1);
    var maxBit = 1024;
    if (cssW * dpr > maxBit) dpr = maxBit / cssW;
    var w = Math.round(cssW*dpr), h = Math.round(cssH*dpr);
    if (w === this.el.width && h === this.el.height && this.w === cssW) return false;
  ...
  var Globe = (function(){
    ...
    var nodes = [];
    (function(){
      var LAT_STEP = 1.7, run = 0, sea3 = 0;
      for (var lat = -86; lat <= 86; lat += LAT_STEP){
        var rl = Math.cos(lat*Math.PI/180);
        var n  = Math.max(1, Math.round(168*rl));
        for (var i=0;i<n;i++){
          var lon = -180 + 360*i/n;
          var l = isLand(lon, lat);
          if (!l && (sea3++ % 3 !== 0)) continue;
          var mark = !!(l && (run++ % 2 === 0));
          nodes.push({
    ...
    var spin = 2.1, vel = 0.055, hover = false, drag = null, tilt = -0.36, vtilt = 0;
    var IDLE = 0.055;
    ...
    cv.addEventListener('pointermove', function(e){
      ...
      look = p;
      if (!drag) return;
      vel  = (p.x - drag.x) / s.u * 16;
      vtilt = -(p.y - drag.y) / s.u * 11;
      tilt  = Math.max(-1.15, Math.min(1.15, tilt + vtilt*0.032));
      drag = p; drag.moved = true;
    });
    function release(e){
      if (drag && !drag.moved){
        var g = unproject(drag.x, drag.y);
        if (g){ pins.push({lat:g.lat, lon:g.lon, t:performance.now()}); }
      }
      drag = null;
    }
    ...
    cv.addEventListener('wheel', function(e){
      e.preventDefault();
      try { parent.postMessage({ __digineomGlobe: 'wheel', deltaY: e.deltaY, deltaX: e.deltaX }, '*'); } catch (err) {}
    }, {passive:false});
    ...
    function draw(now, dt){
      ...
      if (!drag){
        var idle = hover ? 0.09 : IDLE;
        vel += (idle - vel) * (1 - Math.exp(-dt/(hover ? 90 : 130)));
        vtilt *= Math.pow(0.82, dt/16);
        tilt += vtilt*dt/1000;
        tilt += (-0.36 - tilt) * Math.min(1, dt/2200);
      }
      spin += vel * dt/1000;
      ...
      sea.length = 0; soil.length = 0; crust.length = 0;
      for (var i=0;i<nodes.length;i++){
        ...
        var depth = Math.min(0.999, z2 + glow*0.55);
        if (!nd.land){ sea.push(px, py, depth); continue; }
        if (nd.mark){ crust.push(px, py, depth); continue; }
        soil.push(px, py, depth);
      }

      var dmin = Math.max(0.32, s.u*0.00125);
      function dots(list, base, gain, grow){
        for (var lvl=0; lvl<6; lvl++){
          var z = (lvl+0.5)/6, dsz = dmin*grow*(0.48 + 0.58*z);
      ...
      dots(sea,  0.10, 0.22, 0.62);
      dots(soil, 0.34, 0.46, 0.95);
      dots(crust, 0.44, 0.50, 1.18);

      var PIN_LIFE = 0.5;
      for (var pi=pins.length-1; pi>=0; pi--){
        var pn = pins[pi];
        var age = (now - pn.t)/1000;
        if (age >= PIN_LIFE){ pins.splice(pi, 1); continue; }
...
function focusStyles(mode: "dark" | "light") {
  const selected = 1;
  const surface = mode === "light" ? "transparent" : "#08090a";
  const themeStyles =
    mode === "light"
      ? `
      :root {
  ...
  return `<style id="threeui-study-focus">
    ...
    html, body, .frame { background: ${surface} !important; }
...
function focusedDocument(mode: "dark" | "light") {
  ...
  return source
    .replace(/<title>[\s\S]*?<\/title>/i, `<title>Globe</title>`)
...
export default function GlobeStudy({
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      if (event.data?.__digineomGlobe !== "wheel") return;
    ...
    >
      <iframe
        ...
        style={{
          ...
          transform:
            boundedScale === 1 ? "translateZ(0)" : `translateZ(0) scale(${boundedScale})`,
