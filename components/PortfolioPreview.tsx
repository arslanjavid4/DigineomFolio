  ...
  return (
    <section className="section-padding bg-white">
      <div className="container-custom">
        ...
        >
          <div>
            <p className="eyebrow mb-5 text-[#1863dc]">Selected work</p>
            <h2 className="display text-6xl md:text-8xl">Real problems.<br />Measurable outcomes.</h2>
          </div>
          <Link href="/projects" className="pill border border-[#17171c] hover:bg-[#eeece7]">All projects <ArrowRight size={16} /></Link>
        </motion.div>
        ...
        <div className="grid gap-x-6 gap-y-14 md:grid-cols-2">
          {allProjects.map((project, index) => (
            <Link key={project.id} href={`/projects/${project.id}`} className={`group ${index % 2 ? 'md:mt-20' : ''}`}>
              <div className="relative mb-6 aspect-[4/3] overflow-hidden rounded-[18px] bg-neutral-100">
